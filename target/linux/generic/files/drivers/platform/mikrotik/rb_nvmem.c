...
static nvmem_cell_post_process_t rb_tlv_read_cb(u16 tag, u8 *buf)
{
	switch (tag) {
	case RB_ID_MAC_ADDRESS_PACK:
...
static int rb_add_cells(struct device *dev, struct nvmem_device *nvmem,
			const size_t magic_hdr_len, const size_t data_len,
			u8 *data)
{
	u32 node, offset = 0, node_len = sizeof(node);
	...
	while (offset < data_len) {
		...
		cell.offset = magic_hdr_len + offset + node_len;
		/*
		...
		cell.read_post_process = rb_tlv_read_cb(tlv_tag, data + offset + node_len);
		 ...
		 */
skip:
		offset += node_len + ALIGN(tlv_len, sizeof(offset));
	}
...
static int rb_parse_table(struct nvmem_layout *layout)
{
	...
	data = devm_kmalloc(dev, mtd_size - hdr_len, GFP_KERNEL);
	if (!data)
		return -ENOMEM;

	ret = nvmem_device_read(nvmem, hdr_len, mtd_size - hdr_len, data);
	if (ret != mtd_size - hdr_len)
		return ret;

	return rb_add_cells(dev, nvmem, hdr_len, mtd_size - hdr_len, data);
}
